...
{
    public partial class FrmAlmacenReubicacionStockMasivo : BaseDevForm, IBaseDevForm
    {
        ...
        RepositoryItemCheckEdit editSeleccionado = new RepositoryItemCheckEdit();
        RepositoryItemCheckEdit editNoSeleccionado = new RepositoryItemCheckEdit();
        private DataTable dtSeleccionado;

        #endregion

        #region Constructores

        public FrmAlmacenReubicacionStockMasivo()
        {
            InitializeComponent();
            PL_Forms = this;
        ...
        public FrmAlmacenReubicacionStockMasivo(object[] parameters) : this()
        {
        ...
        private void InicializarDataTables()
        {
            ...
            dtMovimientoDetalle.Columns.Add("nro_etiqueta", typeof(string));
            dtMovimientoDetalle.Columns.Add("eet_descripcion", typeof(string));
            dtMovimientoDetalle.Columns.Add("bodega", typeof(string));
            dtMovimientoDetalle.Columns.Add("articulo", typeof(string));
            dtMovimientoDetalle.Columns.Add("lote", typeof(string));
            dtMovimientoDetalle.Columns.Add("eal_kgn", typeof(decimal));
            dtMovimientoDetalle.Columns.Add("eal_kgb", typeof(decimal));
            dtMovimientoDetalle.Columns.Add("eal_kgt", typeof(decimal));
        ...
        {
            dtSeleccionado = new DataTable();

            dtSeleccionado.Columns.Add("nro_etiqueta", typeof(string));
            dtSeleccionado.Columns.Add("eet_descripcion", typeof(string));
            dtSeleccionado.Columns.Add("bodega", typeof(string));
            dtSeleccionado.Columns.Add("articulo", typeof(string));
            dtSeleccionado.Columns.Add("lote", typeof(string));
            dtSeleccionado.Columns.Add("eal_kgn", typeof(decimal));
            dtSeleccionado.Columns.Add("eal_kgb", typeof(decimal));
            dtSeleccionado.Columns.Add("eal_kgt", typeof(decimal));
            dtSeleccionado.Columns.Add("ubi_codigo", typeof(int));
            dtSeleccionado.Columns.Add("ubi_descripcion", typeof(string));

            dtSeleccionado.PrimaryKey = new DataColumn[] { dtSeleccionado.Columns["nro_etiqueta"] };
            grv_Detalle.VisibleColumns[0].OptionsColumn.ShowCaption = false;
        ...
        private void Buscar()
        {
            try
            {
                ...
                if (PostBack)
                {
                    dgv_Detalle.Fn_MostrarCargando();
                }
                else
                {
                ...
                object[] parameters = {
                    ...
                    Parser.GetString(txt_lote.Text),
                    Parser.GetString(txt_bodega.Text),
                };
                DataTable dtInformacion = new Principal().RetornaBusquedaEtiquetasReubicacion(parameters);
                dgv_Detalle.DataSource = dtInformacion;
            }
            catch (Exception ex)
            {
            ...
            {
                dgv_Detalle.Fn_OcultarCargando();
            }
        ...
        private void Grabar()
        {
            ...
            try
            {
                ...
                DataTable dtBultos = new DataTable();
                dtBultos.Columns.Add("nro_etiqueta", typeof(string));
                //dtBultos.Columns.Add("ubi_codigo", typeof(int));

                foreach (DataRow row in dtSeleccionado.Rows)
                {
                    //dtBultos.Rows.Add(row["nro_etiqueta"], row["ubi_codigo"]);
                    string nroEtiqueta = row["nro_etiqueta"].ToString();  // Obtener nro_etiqueta correcto
                    dtBultos.Rows.Add(nroEtiqueta);
                }
                ...
                string[] resultado = new Principal().RegistrarReubicacionEtiquetasBloque(parameters);
        ...
        private void grv_lineas_CustomRowCellEdit(object sender, CustomRowCellEditEventArgs e)
        {
            if (dgv_Detalle.Fn_ObtenerNumeroFilas() > 0)
            {
                if (e.Column.FieldName == "DX$CheckboxSelectorColumn")
                {
                    if (grv_Detalle.GetRowCellValue(e.RowHandle, "nro_etiqueta").ToString() != "D")
                    {
                        e.RepositoryItem = editNoSeleccionado;
                        //Dialog.MostrarMensajeAdvertencia("No se puede seleccionar esta línea.");
                    }
                    else
                    {
                        e.RepositoryItem = editSeleccionado;
                    }
                }
            }
        ...
        private void grv_stock_disponible_SelectionChanged(object sender, DevExpress.Data.SelectionChangedEventArgs e)
        {
            //Se valida que lo seleccionado cumpla con las reglas para seleccionarse:
            GridView view = sender as GridView;
            int[] filas_seleccionadas = view.GetSelectedRows();

            dtSeleccionado.Rows.Clear();

            //Se verifica que solo se seleccione lo que no se ha facturado
            foreach (int fila in filas_seleccionadas)
            {
                if (dgv_Detalle.Fn_ObtenerValorFila(fila, "eet_descripcion").ToString() != "Disponible")
                {
                    grv_Detalle.UnselectRow(fila);
                    // Dialog.MostrarMensajeAdvertencia("No se puede seleccionar esta etiqueta.");
                }
                else
                {
                    string nro_etiqueta = grv_Detalle.GetRowCellValue(fila, "nro_etiqueta").ToString();

                    // Verifica si ya existe en dtSeleccionado
                    if (dtSeleccionado.Select($"nro_etiqueta = '{nro_etiqueta}'").Length == 0)
                    {
                        DataRow nuevaFila = dtSeleccionado.NewRow();
                        nuevaFila["nro_etiqueta"] = grv_Detalle.GetRowCellValue(fila, "nro_etiqueta");
                        nuevaFila["eet_descripcion"] = grv_Detalle.GetRowCellValue(fila, "eet_descripcion");
                        nuevaFila["bodega"] = grv_Detalle.GetRowCellValue(fila, "bodega");
                        nuevaFila["articulo"] = grv_Detalle.GetRowCellValue(fila, "articulo");
                        nuevaFila["lote"] = grv_Detalle.GetRowCellValue(fila, "lote");
                        nuevaFila["eal_kgn"] = grv_Detalle.GetRowCellValue(fila, "eal_kgn");
                        nuevaFila["eal_kgb"] = grv_Detalle.GetRowCellValue(fila, "eal_kgb");
                        nuevaFila["eal_kgt"] = grv_Detalle.GetRowCellValue(fila, "eal_kgt");
                        nuevaFila["ubi_codigo"] = grv_Detalle.GetRowCellValue(fila, "ubi_codigo");
                        nuevaFila["ubi_descripcion"] = grv_Detalle.GetRowCellValue(fila, "ubi_descripcion");

                        dtSeleccionado.Rows.Add(nuevaFila);
                    }
                }
            }
        ...
        private void grv_stock_disponible_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            //if (!(sender is GridView view))
            //    return;

            //view.UpdateCurrentRow(); //This line is solve the problem...  

            if (e.Column == nro)
            {
                string articulo = grv_Detalle.GetRowCellValue(e.RowHandle, "ARTICULO").ToString();
                string bodega = grv_Detalle.GetRowCellValue(e.RowHandle, "BODEGA").ToString();
                string embarque = grv_Detalle.GetRowCellValue(e.RowHandle, "EMBARQUE").ToString();

                //CargarDetalleEtiquetasTotal(articulo, bodega, embarque);
            }
        }
        ...
        private void btnSeleccionarTodo_Click(object sender, EventArgs e)
        {
            if (dgv_Detalle.Fn_ObtenerNumeroFilas() == 0)
            {
                Dialog.MostrarMensajeAdvertencia("Primero realice una búsqueda");
                return;
            }
            for (int i = 0; i < grv_Detalle.DataRowCount; i++)
            {
                if (!Parser.GetBoolean(dgv_Detalle.Fn_ObtenerValorFila(i, "chk")))
                {
                    if (dgv_Detalle.Fn_ObtenerValorFila(i, "eet_descripcion").ToString() == "Disponible")
                    {
                        grv_Detalle.SelectRow(i);

                        string nro_etiqueta = Parser.GetString(grv_Detalle.GetRowCellValue(i, "nro_etiqueta").ToString());
                        string eet_etiqueta = Parser.GetString(grv_Detalle.GetRowCellValue(i, "eet_descripcion").ToString());
                        string bodega = Parser.GetString(grv_Detalle.GetRowCellValue(i, "bodega").ToString());
                        string articulo = Parser.GetString(grv_Detalle.GetRowCellValue(i, "articulo").ToString());
                        string lote = Parser.GetString(grv_Detalle.GetRowCellValue(i, "lote").ToString());
                        decimal peso_neto = Parser.GetDecimal(grv_Detalle.GetRowCellValue(i, "eal_kgn").ToString());


                        var existing = dtSeleccionado.Rows.Find(new object[] { nro_etiqueta });
                        if (existing == null)
                            dtSeleccionado.Rows.Add(articulo, bodega, lote, nro_etiqueta, peso_neto);
                    }
                }
            }
            ...
            if (dgv_Detalle.Fn_ObtenerNumeroFilasSeleccionadas() == 0)
            {
                Dialog.MostrarMensajeAdvertencia("Ninguna etiqueta cumple con los requisitos para ser seleccionado. Por favor, verifique.");
            }

            dtSeleccionado.AcceptChanges();
        }
        ...
        private void btnLimpiar_Click(object sender, EventArgs e)
        {
            if (dgv_Detalle.Fn_ObtenerNumeroFilas() == 0)
            {
                Dialog.MostrarMensajeAdvertencia("Debe realizar una búsqueda antes de realizar esta operación.");
                return;
            }
            grv_Detalle.ClearSelection();
            dtSeleccionado.Rows.Clear();
        }
