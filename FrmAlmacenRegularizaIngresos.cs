...
{
    public partial class FrmAlmacenRegularizaIngresos : BaseDevForm, IBaseDevForm
    {
        ...
        public int vg_tar_codigo = 0;

        #endregion

        #region Constructores

        public FrmAlmacenRegularizaIngresos()
        {
            InitializeComponent();
            PL_Forms = this;
        ...
        private void Inicializar()
        {
            ...
            if (vg_tar_codigo > 0)
            {
                cbo_tar_codigo.EditValue = Convert.ToByte(vg_tar_codigo);
            }
        ...
        private void Buscar()
        {
            try
            {
                ...
                object[] parametros = {
                    ...
                    Parser.GetInt(cbo_tar_codigo.EditValue.ToString()),
                    Parser.GetInt(cbo_tmo_codigo.EditValue.ToString()),
                    Parser.GetString(cbo_bpu_modulo.EditValue.ToString()),
                    PL_UsuarioE.PL_UsuarioCodigo
                };

                dgv_Pesado.Fn_LimpiarGrilla();
                DataTable dt = new Principal().RetornaRegularizacionIngresos(parametros);
                ...
                dgv_Pesado.NombreArchivoExportado = "Reporte de ingresos pendientes de regularizar";
            }
            catch (Exception ex)
            {
        ...
        private void FrmAlmacenGeneraDocumentoInventario_Click_Aceptar(object sender, EventArgs e)
        {
            ...
            //Se valida los documentos en EXACTUS
            object[] parameters = { dtUbicacionPendiente };
            DataTable dtvalida = new Principal().VerificarUbicacionMovimientoIngreso(parameters);
        ...
        private void grv_Pesado_RowCellClick(object sender, RowCellClickEventArgs e)
        {
        ...
        private void FrmAlmacenRegularizaIngresos_Click_Personalizado10(object sender, EventArgs e)
        {
            int linea = grv_Pesado.FocusedRowHandle;
            string documento_inventario = dgv_Pesado.Fn_ObtenerValorFila(linea, "pea_documento_inv").ToString();
            ...
            using (FrmAlmacenRevertirIngreso frm = new FrmAlmacenRevertirIngreso())
            {
                frm.documento_inventario = documento_inventario;
                frm.Fn_AbrirFormulario(frm, this, true);
                if (frm.DialogResult == DialogResult.Yes)
                {
                    Buscar();
        ...
        private void btnReubicacionBloque_Click(object sender, EventArgs e)
        {
                    ...
                    using (FrmAlmacenReubicacionStockMasivo frx = new FrmAlmacenReubicacionStockMasivo())
                    {
                        ...
                        if (frx.DialogResult == DialogResult.Yes)
                        {
